...
export default function SaleDetailPage() {
  ...
  const schedules = schedulesForSale(sale.id)
  const history = paymentsForSale(sale.id)
  const firstUnpaid = useMemo(() => schedules.find((s) => s.status !== "PAID"), [schedules])
      ...
      <div className="p-6 overflow-y-auto">
        ...
        <div className="grid gap-6 lg:grid-cols-2 mb-6">
          ...
          <div className="surface-card card-rounded border border-default p-6 shadow-sm">
            <p className="t-micro-bold text-soft case-upper tracking-label mb-4">Customer</p>
            <div className="flex items-start gap-4">
              <InitialsAvatar name={customer?.fullName ?? ""} size="lg" />
              <div className="flex-1 min-w-0">
                <h3 className="t-section text-main mb-1">{customer?.fullName}</h3>
          ...
          <div className="surface-card card-rounded border border-default p-6 shadow-sm relative">
            <div className="flex items-start justify-between mb-4">
              <p className="t-micro-bold text-soft case-upper tracking-label">Sale Summary</p>
        ...
        <section className="surface-card modal-rounded border border-default shadow-sm mb-6 overflow-hidden">
          <div className="flex items-center justify-between px-6 h-14 border-b border-default">
            ...
            <span className="surface-brand-soft text-brand t-caption-bold px-3 py-1 circle-rounded">
              {stat.paidCount} of {stat.totalCount} paid · {formatCurrency(stat.outstanding)} remaining
            </span>
          </div>
            ...
            <table className="w-full">
              ...
              <tbody>
                {schedules.map((s) => {
                  ...
                  return (
                    <tr key={s.id} className={`border-t border-default ${rowClass}`}>
                      ...
                      <td className="px-6 py-3 align-text-right table-text">{formatCurrency(s.paidAmount)}</td>
                      ...
                      <td className="px-6 py-3 align-text-right">
                        {!isPaid ? (
                          <Button
                            ...
                            className="t-caption-bold control-rounded border-brand-soft text-brand hover:bg-[var(--primary-light)] bg-transparent"
                          >
                            Record Payment
                          </Button>
                        ) : (
          ...
          {history.length === 0 ? (
            ...
            <div className="relative pl-8">
              ...
              <ul className="flex flex-col gap-6">
                {history.map((p) => {
                  const sched = schedules.find((s) => s.id === p.installmentScheduleId)
                  return (
                    <li key={p.id} className="relative">
                      <span
                        className="absolute -left-[26px] top-1.5 h-3 w-3 circle-rounded ring-4 ring-[var(--card-bg)]"
                        style={{ backgroundColor: "var(--primary)" }}
                        aria-hidden
                      />
                      <div className="flex flex-wrap items-center gap-3 mb-1">
                        <span className="t-meta-bold text-main">{formatDate(p.paidDate)}</span>
                        <span className="t-display fw-black text-brand">{formatCurrency(p.paidAmount)}</span>
                        {sched && (
                      ...
                      <p className="t-caption text-soft">Recorded by {p.recordedBy}</p>
                      {p.notes && (
...
function KV({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div>
      <p className="t-micro-bold text-soft case-upper tracking-label mb-1">{label}</p>
      <p className={`t-meta-bold ${highlight ? "text-brand" : "text-main"}`}>{value}</p>
    </div>
  )
}
