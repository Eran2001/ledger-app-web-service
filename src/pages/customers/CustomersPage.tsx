...
import { customers, sales, installmentSchedules } from '@/data/dummy'
...
export const CustomersPage: React.FC = () => {
  ...
  const enrichedCustomers = customers.map(c => {
    ...
    const schedules = installmentSchedules.filter(is => customerSaleIds.includes(is.saleId))
        ...
        <div className="mb-6 flex flex-wrap items-center gap-4">
          ...
          <div className="flex gap-1 bg-slate-200/50 p-1 rounded-lg">
            {(['all', 'active', 'overdue'] as const).map(tab => (
              ...
              >
                {tab === 'active' ? 'Active Sales' : tab === 'overdue' ? 'Has Overdue' : 'All Customers'}
              </button>
            ))}
          </div>
        ...
        <div className="bg-white rounded-xl border border-[#E2E8F0] shadow-sm overflow-hidden">
          {filteredCustomers.length > 0 ? (
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                              <span className="text-sm font-semibold text-[#0F172A] group-hover:text-[#4F46E5]">{c.fullName}</span>
                              <span className="text-[11px] text-[#94A3B8] capitalize">{c.address.split(',')[1]?.trim() || 'Colombo'}</span>
                            </div>
                          </div>
                        </td>
                        <td className="px-5 py-4 text-sm text-[#475569] font-mono">{c.nic}</td>
                        <td className="px-5 py-4 text-sm text-[#475569]">{c.phone}</td>
                        <td className="px-5 py-4 text-sm font-medium text-center">
                           <span className={cn(c.activeSalesCount > 0 ? "text-[#0F172A]" : "text-[#94A3B8]")}>
                             {c.activeSalesCount}
                           </span>
                        </td>
                        <td className="px-5 py-4 text-sm font-bold text-[#0F172A]">
                          {c.outstanding > 0 ? formatCurrency(c.outstanding) : <span className="text-[#94A3B8]">Paid</span>}
                        </td>
                        <td className="px-5 py-4">
            ...
            <EmptyState 
              ...
              subtitle={search ? "We couldn't find any customers matching your search query." : "Add your first customer to start tracking installment sales."}
