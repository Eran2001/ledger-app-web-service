...
export default function OverduePage() {
  ...
  const filtered = useMemo(() => {
    return allRows.filter((r) => {
      if (tab === "1-30 days" && (r.daysOverdue < 1 || r.daysOverdue > 30)) return false
      if (tab === "31-60 days" && (r.daysOverdue < 31 || r.daysOverdue > 60)) return false
  ...
  const allSelected = filtered.length > 0 && filtered.every((r) => selected.has(r.scheduleId))
  ...
  return (
    <div className="flex flex-col h-full surface-page">
      <TopBar
        ...
        primaryAction={{ onClick: () => Notification.success("Reminders queued for all overdue customers"), icon: Send, label: "Send All Reminders" }}
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="card-base p-5 border-l-4 border-start-danger">
          ...
          <p className="t-kpi-lg text-danger mt-2">{formatCurrency(totalOverdue)}</p>
          <p className="t-caption text-faint mt-1">Across {allRows.length} installments</p>
        </div>
        <div className="card-base p-5 border-l-4 border-start-danger">
          ...
          <p className="t-caption text-soft mt-1">{longest?.customerName ?? "No overdues"}</p>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr>
                ...
                <th className="table-header text-left px-4 py-3">Days Overdue</th>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-4 py-12 align-text-center text-faint t-meta">
                ...
                filtered.map((r) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <p className="table-title-text truncate">{r.customerName}</p>
                            <p className="t-micro text-faint mono-text">{r.customerPhone}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <span className={`fw-black ${sev.text}`}>{r.daysOverdue} days</span>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1 justify-end">
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => Notification.success(`Reminder sent to ${r.customerName}`)}
                            className="gap-1.5"
                          >
          ...
          <div className="app-sidebar modal-rounded shadow-2xl px-4 py-3 flex items-center gap-4">
            <div className="flex items-center gap-3">
              <div className="circle-rounded surface-brand h-8 w-8 flex items-center justify-center">
                <span className="t-meta-bold text-inverse">{selected.size}</span>
              </div>
              <span className="t-meta-bold app-sidebar-text">
                {selected.size} {selected.size === 1 ? "Customer" : "Customers"} selected for reminders
              </span>
            </div>
            <div className="flex items-center gap-2">
              <Button
                ...
                className="app-sidebar-text-soft hover:bg-white/10 hover:text-white"
              >
                Cancel
              </Button>
              <Button
                onClick={() => {
                  Notification.success(`WhatsApp reminders sent to ${selected.size} customers`)
                  setSelected(new Set())
                }}
