...
import { customers, products } from '@/data/dummy'
...
export const NewSalePage: React.FC = () => {
  ...
  const previewSchedule = useMemo(() => {
    ...
    for (let i = 1; i <= months; i++) {
        schedule.push({
            ...
            amount: i === months ? remainingBalance - (monthlyAmount * (months - 1)) : monthlyAmount
        })
    }
    return schedule
  ...
  return (
    <div className="flex flex-col h-full bg-[#F8FAFC]">
      <Topbar 
        pageTitle="New Sale" 
        pageSubtitle="Create installment sale" 
      />
        ...
        <div className="grid grid-cols-1 xl:grid-cols-5 gap-8">
          ...
          <div className="xl:col-span-3 bg-white rounded-2xl border border-[#E2E8F0] p-8 shadow-sm">
             <div className="flex items-center gap-3 border-l-4 border-[#4F46E5] pl-4 mb-8">
                <h3 className="font-bold text-lg text-[#0F172A]">Customer & Product</h3>
             </div>

             <div className="space-y-6">
                <div className="space-y-2">
                  <Label className="text-slate-700 font-semibold">Select Customer</Label>
                ...
                <div className="space-y-2">
                  <Label className="text-slate-700 font-semibold">Select Product</Label>
                  ...
                  {selectedProduct && (
                    <p className="text-[11px] text-[#94A3B8] font-medium">Standard Price: {formatCurrency(selectedProduct.basePrice)}</p>
                  )}
                </div>

                <div className="pt-6 border-t border-slate-100 mt-8">
                    <div className="flex items-center gap-3 border-l-4 border-[#4F46E5] pl-4 mb-8">
                        <h3 className="font-bold text-lg text-[#0F172A]">Sale Terms</h3>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                            <Label className="text-slate-700 font-semibold">Sold Price (LKR)</Label>
                            <Input 
                                type="number" 
                                value={soldPrice} 
                        ...
                        <div className="space-y-2">
                            <Label className="text-slate-700 font-semibold">Down Payment (LKR)</Label>
                            <Input 
                                type="number" 
                                value={downPayment} 
                        ...
                        <div className="space-y-2">
                            <Label className="text-slate-700 font-semibold">Number of Months</Label>
                        ...
                        <div className="space-y-2">
                            <Label className="text-slate-700 font-semibold">Sale Date</Label>
                            <Input 
                                type="date" 
                                value={saleDate} 
                    ...
                    <div className="mt-6 space-y-2">
                        <Label className="text-slate-700 font-semibold">Notes (Optional)</Label>
                        <Textarea 
          ...
          <div className="xl:col-span-2 space-y-6">
            <div className="bg-white rounded-2xl border border-[#E2E8F0] p-6 shadow-sm sticky top-6">
                <div className="flex items-center gap-2 mb-6 pb-4 border-b border-slate-50">
                    ...
                    <h3 className="font-bold text-[#0F172A]">Installment Preview</h3>
                </div>

                <div className="space-y-4 mb-8">
                    <div className="flex justify-between items-center text-sm font-medium">
                        <span className="text-[#475569]">Sold Price</span>
                        <span className="text-[#0F172A]">{formatCurrency(soldPrice)}</span>
                    </div>
                    <div className="flex justify-between items-center text-sm font-medium">
                        <span className="text-[#475569]">Down Payment</span>
                        <span className="text-[#15803D] border-b border-dashed border-green-200 pb-0.5">-{formatCurrency(downPayment)}</span>
                    </div>
                    <div className="flex justify-between items-center py-3 border-y border-slate-50">
                        <span className="text-sm font-bold text-[#0F172A]">Remaining Balance</span>
                        <span className="text-base font-black text-[#0F172A]">{formatCurrency(remainingBalance)}</span>
                    </div>
                    <div className="bg-blue-50/50 rounded-xl p-4 flex justify-between items-center">
                        <span className="text-sm font-bold text-[#475569]">Monthly Payment</span>
                        <span className="text-lg font-black text-[#4F46E5]">{formatCurrency(monthlyAmount)}</span>
                    </div>
                    <div className="flex justify-between items-center text-sm font-medium">
                ...
                <div className="space-y-3">
                   <h4 className="text-[10px] text-[#94A3B8] font-bold uppercase tracking-widest pl-1">Generated Schedule</h4>
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {previewSchedule.map((row) => (
                                <tr key={row.num} className="hover:bg-slate-50 transition-colors">
                                    <td className="py-2.5 pl-2 text-slate-400 font-bold">{row.num}</td>
                                    <td className="py-2.5 text-[#475569]">{formatDate(row.due)}</td>
                                    <td className="py-2.5 text-right pr-2 text-[#0F172A] font-bold">{formatCurrency(row.amount)}</td>
                                </tr>
                            ))}
                        </tbody>
                      </table>
