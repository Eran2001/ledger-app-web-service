...
export const SalesPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'all' | 'ACTIVE' | 'OVERDUE' | 'COMPLETED' | 'WRITTEN_OFF'>('all')
  ...
  const enrichedSales = sales.map(s => {
    ...
    const outstanding = schedules.reduce((acc, curr) => acc + (curr.expectedAmount - curr.paidAmount), 0)
    const nextDueSchedule = schedules.find(is => is.status === 'PENDING' || is.status === 'PARTIALLY_PAID' || is.status === 'OVERDUE')
    ...
    <div className="flex flex-col h-full bg-[#F8FAFC]">
      ...
      <div className="p-6 overflow-y-auto">
        ...
        <div className="mb-6 flex gap-1 bg-slate-200/50 p-1 rounded-lg w-max">
          {(['all', 'ACTIVE', 'OVERDUE', 'COMPLETED', 'WRITTEN_OFF'] as const).map(tab => (
            <button
              key={tab}
            ...
            >
              {tab === 'all' ? 'All Sales' : tab.toLowerCase().replace('_', ' ')}
            </button>
          ))}
        </div>
        ...
        <div className="mb-4 flex flex-wrap items-center gap-3">
          ...
          <div className="flex gap-2">
            <Input type="date" className="h-10 w-40 border-[#E2E8F0] text-xs font-medium" />
            <span className="text-slate-300 self-center">to</span>
            <Input type="date" className="h-10 w-40 border-[#E2E8F0] text-xs font-medium" />
          </div>
          <Button variant="outline" className="ml-auto h-10 gap-2 border-[#E2E8F0] shadow-sm">
        ...
        <div className="bg-white rounded-xl border border-[#E2E8F0] shadow-sm overflow-hidden">
          {filteredSales.length > 0 ? (
                    ...
                    {filteredSales.map((s) => (
                      <tr 
                        ...
                        className={cn(
                          ...
                          s.computedStatus === 'OVERDUE' && "bg-[#FFF8F8] border-l-2 border-l-red-500"
                        )}
                      ...
                      >
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm font-semibold text-[#0F172A] group-hover:text-[#4F46E5]">{s.customerName}</span>
                          </div>
                        </td>
                        <td className="px-5 py-4">
                          <div className="flex flex-col">
                            <span className="text-sm font-semibold text-[#475569]">{s.productName}</span>
                            <span className={cn(
                                "text-[10px] uppercase font-bold tracking-wider mt-0.5",
                                s.category === 'Electronics' ? "text-indigo-500" :
                                s.category === 'Appliances' ? "text-teal-500" :
                                s.category === 'Furniture' ? "text-amber-500" : "text-slate-400"
                            )}>
                              {s.category}
                            </span>
                          </div>
                        </td>
                        <td className="px-5 py-4 text-sm text-[#475569]">{formatCurrency(s.soldPrice)}</td>
                        <td className="px-5 py-4 text-sm font-bold text-[#0F172A]">{formatCurrency(s.outstanding)}</td>
                        <td className="px-5 py-4 text-sm text-[#475569] font-medium">{formatCurrency(s.monthlyAmount)}</td>
            ...
            <EmptyState 
              ...
              subtitle={search ? "Try adjusting your search or filters." : "Create a new sale to generate an installment schedule."}
