      ...
      <div className="p-6 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-card card-rounded border border-border border-l-4 border-l-destructive p-5 shadow-sm">
                <p className="t-micro-bold text-body uppercase tracking-wider mb-1">Overdue Customers</p>
                <div className="t-kpi-lg fw-black text-heading">{enrichedOverdue.length}</div>
            </div>
            <div className="bg-card card-rounded border border-border border-l-4 border-l-destructive p-5 shadow-sm">
                <p className="t-micro-bold text-body uppercase tracking-wider mb-1">Total Overdue Amount</p>
            ...
            <div className="bg-card card-rounded border border-border border-l-4 border-l-destructive p-5 shadow-sm">
                <p className="t-micro-bold text-body uppercase tracking-wider mb-1">Longest Overdue</p>
                <div className="t-kpi-lg fw-black text-heading">{Math.max(...enrichedOverdue.map(is => is.daysOverdue), 0)} Days</div>
                <p className="t-micro-bold text-hint mt-1">{enrichedOverdue.filter(is => is.daysOverdue === Math.max(...enrichedOverdue.map(x => x.daysOverdue), 0))[0]?.customerName}</p>
            </div>
        </div>
                    ...
                    {filteredOverdue.map((is) => (
                      <tr
                        ...
                        className={cn(
                          ...
                          is.daysOverdue > 60 ? "bg-overdue-row" : "hover:bg-surface"
                        )}
                      >
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                              <span className="t-body fw-semibold text-heading">{is.customerName}</span>
                              <span className="t-micro text-hint">{is.phone}</span>
                            </div>
                          </div>
                        </td>
                        <td className="px-5 py-4 t-body text-body">{is.productName}</td>
                        <td className="px-5 py-4 t-body text-body">{formatDate(is.dueDate)}</td>
                        <td className="px-5 py-4">
                          <span className={cn(
                             "t-caption fw-black uppercase tracking-tight",
                             is.daysOverdue > 60 ? "text-destructive" : is.daysOverdue > 30 ? "text-warning" : "text-warning"
                          )}>
                             {is.daysOverdue} Days
                          </span>
                        </td>
                        <td className="px-5 py-4 t-body fw-bold text-heading">{formatCurrency(is.expectedAmount - is.paidAmount)}</td>
        ...
        {selectedIds.length > 0 && (
            <div className="fixed bottom-6 left-[250px] right-6 z-50 animate-in slide-in-from-bottom duration-300">
                <div className="bg-sidebar text-on-dark p-4 rounded-2xl shadow-2xl flex items-center justify-between border border-sidebar-border">
                    <div className="flex items-center gap-4 px-2">
                        <div className="w-8 h-8 rounded-full bg-primary flex items-center justify-center t-body fw-bold">
                            {selectedIds.length}
                        </div>
                        <span className="t-body fw-semibold">Customers selected for reminders</span>
                    </div>
                    <div className="flex gap-2">
                        <Button variant="ghost" className="text-on-dark hover:bg-sidebar-accent" onClick={() => setSelectedIds([])}>Cancel</Button>
                        <Button className="bg-primary hover:bg-primary-dark">Send WhatsApp Reminders</Button>
                    </div>
                </div>
            </div>
        )}
