...
const FeatureCard: React.FC<{ icon: React.ElementType, title: string, subtitle: string }> = ({ icon: Icon, title, subtitle }) => (
    ...
    <div className="flex min-h-screen bg-white">
      ...
      <div className="w-full lg:w-[45%] flex items-center justify-center p-8 bg-white">
        <div className="max-w-sm w-full">
          <div className="flex items-center gap-2 mb-12">
            <div className="bg-[#4F46E5] text-white px-2 py-1 rounded-lg text-xs font-bold">ST</div>
            <span className="text-xl font-bold text-[#0F172A]">Silva Traders</span>
          </div>

          <div className="mb-8">
            <h2 className="text-3xl font-bold text-[#0F172A]">Welcome back</h2>
            <p className="text-slate-500 mt-2">Sign in to your admin panel to manage your business.</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div className="flex justify-end">
              <button type="button" className="text-xs font-semibold text-[#4F46E5] hover:underline">
                Forgot password?
              </button>
            </div>
      ...
      <div className="hidden lg:flex flex-1 bg-gradient-to-br from-[#1E1B4B] via-[#312E81] to-[#1E3A5F] px-16 py-20 flex-col justify-center relative overflow-hidden">
        ...
        <div className="mt-auto relative z-10">
          <p className="text-white/20 text-xs font-mono">SILVA TRADERS v1.0.0 · © 2026 Admin Panel</p>
        </div>
      </div>
    </div>
  )
