...
export default function NewSalePage() {
  ...
  const monthly = months > 0 && remaining > 0 ? Math.ceil(remaining / months) : 0
  ...
  function submit(e: React.FormEvent) {
    ...
    toast.success("Sale created and schedule generated.")
    setTimeout(() => router.push("/sales"), 600)
  }

  return (
    <div className="flex flex-col h-full surface-page">
      <TopBar pageTitle="New Sale" pageSubtitle="Create a new installment sale" />
          ...
          <div className="xl:col-span-3">
            <div className="surface-card modal-rounded border border-default p-8 shadow-sm flex flex-col gap-7">
              ...
              <div className="flex flex-col gap-2">
                ...
                {product && (
                  <p className="t-caption text-soft">
                    Standard price: <span className="fw-bold text-main">{formatCurrency(product.basePrice)}</span>
                  </p>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div className="flex flex-col gap-2">
                  <Label className="t-meta-bold text-main">Sold price (LKR)</Label>
                  <Input
                    type="number"
                    value={soldPrice || ""}
                ...
                <div className="flex flex-col gap-2">
                  <Label className="t-meta-bold text-main">Down payment (LKR)</Label>
                  <Input
                    type="number"
                    value={downPayment || ""}
                ...
                <div className="flex flex-col gap-2">
                  <Label className="t-meta-bold text-main">Number of months</Label>
            ...
            <div className="surface-card modal-rounded border border-default p-6 shadow-sm sticky top-6">
              ...
              <div className="flex flex-col">
                ...
                <div className="flex justify-between items-center py-3 border-y border-dashed border-success-soft">
                  ...
                  <span className="t-meta-bold text-success-role">−{formatCurrency(downPayment)}</span>
                </div>
                <div className="flex justify-between items-center py-3 border-b border-default">
                  <span className="t-meta-bold text-main">Remaining Balance</span>
                  <span className="t-display text-main">{formatCurrency(remaining)}</span>
                </div>
                <div className="surface-brand-soft card-rounded p-4 mt-4 mb-3 flex justify-between items-center">
                  <span className="t-meta-bold text-brand">Monthly Payment</span>
                  <span className="t-section text-brand">{formatCurrency(monthly)}</span>
                </div>
                <Row label="Duration" value={`${months} months`} />
              </div>

              <div className="mt-5 pt-5 border-t border-default">
                <p className="t-micro-bold text-soft case-upper tracking-label mb-2">Generated Schedule</p>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="t-micro text-faint case-upper tracking-label">
                        ...
                        <th className="px-2 py-1.5 align-text-left">Due Date</th>
                    ...
                    <tbody>
                      {schedule.length === 0 ? (
                        <tr>
                          <td colSpan={3} className="px-2 py-6 align-text-center t-caption text-faint">
                        ...
                        schedule.map((s) => (
                          <tr key={s.n} className="border-t border-default">
                            <td className="px-2 py-2 t-caption text-faint">{s.n}</td>
                            <td className="px-2 py-2 t-caption text-soft">{formatDate(s.due)}</td>
